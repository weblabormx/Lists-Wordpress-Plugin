...
$argv[1] = 'C:\xampp\htdocs\wordpress\wp-content\plugins\list-for-articles';
...
function print_report($unused_functions)
{
	...
	foreach ( $unused_functions as $function => $locations )
	{
		...
		{
			echo "<tr><td>'$function'</td><td>{$location['file']}</td><td>{$location['line']}</td></tr>";
			$count++;
		}
	}
	echo "</table>";
	echo "=======================================<br />";
	echo "Found $count unused function" . (($count == 1) ? '' : 's') . ".<br /><br />";
}
